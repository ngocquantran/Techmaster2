...
async function getCourseIdFromTopic() {
  try {
    ...
    switch (res.data.course.category.id) {
      case 1:
        keyAPI = "vocabs";
        $btnLearning.attr("data-bs-target", "#choosewordmodel");
        $btnLearning.attr("data-bs-toggle", "modal");
        $btnLearning.attr("href", `/learn.html?id=${topicId}`);
        $btnTesting.attr("href", `/test.html?id=${topicId}`);
        break;
      case 2:
        keyAPI = "sens";
        $btnLearning.attr("data-bs-target", "");
        $btnLearning.attr("data-bs-toggle", "");
        $btnLearning.attr("href", `/sen_learn.html?id=${topicId}`);
        $btnTesting.attr("href", `/sen_test.html?id=${topicId}`);
        break;
    }
    console.log(keyAPI);
  } catch (error) {
...
async function getTestResultOfNowStage() {
  try {
    let res = await axios.get(
      `${URL_API}/test/${topicId}/${keyAPI}/test-result`
    );
...
async function getUserRank() {
  try {
    let res = await axios.get(
      `${URL_API}/test/${topicId}/test-result/user-rank`
    );
    console.log(res.data);
...
async function getUserTopicState() {
  try {
    let res = await axios.get(
      `${URL_API}/test/${topicId}/test-result/topic-state`
    );
    console.log(res.data);
    let state = res.data;
...
async function getTopicRecords() {
  try {
    let res = await axios.get(`${URL_API}/test/${topicId}/test-result/record`);
    console.log(res.data);
...
async function getAllComment() {
  try {
    let res = await axios.get(`${URL_API}/topic/${topicId}/comments`);
    comments = res.data;
      ...
      comment.idParent != null
        ...
        : '"'
              ...
              <div class="item-wrapper">
                ...
                <div class="item-discussion">${comment.message}</div>
                ${
                  comment.idParent == null
...
async function createComment(comment) {
  try {
    let res = await axios.post(
      `${URL_API}/topic/${topicId}/create_comment`,
      comment
    );
    getAllComment();
  } catch (error) {
