...
public class EmployerController {
    ...
    public String goToEditEmployer(Model model, @PathVariable String id) {
        ...
        employerForm.setCompanyName(employer.getCompanyName());
        employerForm.setCompanyType(employer.getCompanyType());
        employerForm.setLocations(employer.getLocations());
        employerForm.setEmail(employer.getEmail());

        model.addAttribute("id", id);
        model.addAttribute("employer", employer);
        model.addAttribute("editEmployer", employerForm);
    ...
    public String handleEditEmployer(@ModelAttribute EmployerForm employerForm, Model model, @PathVariable String id, BindingResult bindingResult) {
        Employer employer = employerRepo.getEmployers().get(id);
        ...
        if (employerForm.getImage().isEmpty()) {
            employerRepo.getEmployers().put(id, new Employer(id, employer.getLogoPath(), employerForm.getCompanyName(), employerForm.getCompanyType(), employerForm.getLocations(), employerForm.getEmail()));
        } else {
            storageService.uploadFile(employerForm.getImage());
            String pathImg = "/img/" + employerForm.getImage().getOriginalFilename() + "";
            EmployerRequest employerRequest = new EmployerRequest(pathImg, employerForm.getCompanyName(), employerForm.getCompanyType(), employerForm.getLocations(), employerForm.getEmail());
            employerRequest.setLogoPath(pathImg);
